...
function DashboardPage() {
  ...
  useEffect(() => {
    if (groupId) fetchFloorHourly(selectedFloor);
  }, [groupId, selectedDate, selectedFloor]);

  return (
    <div style={{ background: '#F4F7FE', minHeight: '100vh', padding: '32px', width: '100%', overflowX: 'hidden' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
        <h2 style={{ color: '#2B3674', margin: 0 }}>대시보드</h2>
        <button onClick={fetchData}
          style={{ padding: '6px 12px', fontSize: '14px', borderRadius: '6px', border: 'none', cursor: 'pointer', backgroundColor: '#2B3674', color: 'white' }}
        >데이터 요청하기</button>
      </div>
      ...
      <div style={{ display:'flex', gap:'8px', marginTop:'40px', flexWrap:'wrap' }}>
        {[1,2,3,4,5,6].map(floor=> (
          <button key={floor} onClick={()=>setSelectedFloor(floor)}
            style={{ padding:'6px 12px', borderRadius:'6px', border: selectedFloor===floor?'none':'1px solid #2B3674', backgroundColor:selectedFloor===floor? '#2B3674':'white', color:selectedFloor===floor?'white':'#2B3674', cursor:'pointer' }}
          >{floor}층</button>
        ))}
      </div>

      <ChartContainer title={`시간별 사용량 (${selectedFloor}층, kWh)`} data={floorHourlyData}/>
    </div>
  );
}
...
function Control({ label, value, onChange, isDate }) {
  return (
    <div style={{ background:'white', borderRadius:'20px', padding:'20px', width:'300px' }}>
      ...
      {isDate ? (
        <input type="date" min="2021-04-12" max="2021-06-30" value={value} onChange={e=>onChange(e.target.value)}
          style={{ width:'100%', padding:'10px', marginTop:'8px', borderRadius:'8px', border:'1px solid #ccc', fontSize:'16px', color:'#fff', backgroundColor:'#000' }}
        />
      ) : (
        <select value={value} onChange={e=>onChange(e.target.value)}
          style={{ width:'100%', padding:'10px', marginTop:'8px', borderRadius:'8px', border:'1px solid #ccc', fontSize:'16px', backgroundColor:'#fff', color:'#2B3674' }}
        >
...
function ChartContainer({ title, data, bar=false }) {
  return (
    <div style={{ background:'white', borderRadius:'20px', marginTop:'20px', padding:'20px', height:'300px' }}>
